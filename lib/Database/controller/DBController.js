...
class DBController {
  ...
  register(db) {
    ...
    return {
      ...
      /**
       * insert or update one row data to table
       * @param {string} tableName - db table name
       * @param {Object} valueObj - insert table json object { 'column name': value }
       */
      insertOrUpdate: async (tableName, { valueObj = {}, where, whereArgs = [] }) => {
        const sql1 = `
          INSERT OR IGNORE INTO  ${tableName} (${Object.keys(valueObj).join(', ')})
          VALUES (${Object.keys(valueObj).map(() => '?').join(', ')})
        `;
        const sql2 = `
          UPDATE ${tableName} SET ${Object.keys(valueObj).join(' = ?, ')} = ?
          where ${where}
        `;
        this._database.run(sql1, Object.values(valueObj));
        this._database.run(sql2, Object.values(valueObj).concat(whereArgs));
      },
      /**
       * update row to table
       * @param {string} tableName - db table name
       * @param {Object} valueObj - insert table json object { 'column name': value }
       */
      update: async (tableName, { valueObj = {}, where, whereArgs = [] }) => {
        const sql = `
          UPDATE ${tableName} SET ${Object.keys(valueObj).join(' = ?, ')} = ?
          where ${where}
        `;
        this._database.run(sql, Object.values(valueObj).concat(whereArgs));
      },
      /**
