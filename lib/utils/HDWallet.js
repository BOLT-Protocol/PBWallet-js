...
class HDWallet {
  constructor({
    mnemonic, passphrase, purpose = 84, currencyType, testnet = false, lang = 'english',
  }) {
  ...
  getAccount(change, index, pbMode = true) {
    const path = (pbMode)
      ? `m/84'/3324'/0'/${change}'/${index}'`
      : `m/${this._purpose}'/0'/0'/${change}'/${index}'`;
    const node = this.hdWallet.derivePath(path);
    return {
      path,
...
const newInstance = ({
  mnemonic, passphrase, purpose, currencyType, testnet, lang,
}) => {
